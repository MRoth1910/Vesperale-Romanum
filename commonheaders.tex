...
\makeindex[name=A,title=index antiphonarum., options=-s A,columns=2,columnseprule]
\makeindex[name=H,title=index hymnorum., options=-s H,columns=2,columnseprule]
%\makeindex[name=P,title=index psalmorum., options=-s P,columns=2,columnseprule]
%%no ready to typeset Pss yet so this style .ist file isn't real yet
\makeindex[name=V,title=index variorum., options=-s V,columns=2,columnseprule]
\makeindex[name=F,title=index festorum., options=-s F, columns=2,columnseprule]
...
\NewDocumentCommand{\idxinscore}{mmmm}{
  %% #1 : office part letter H, A, V
  %% #2 :  the indexed name of the piece  
  %% #3 : mode
  %% #4   name of the score file
...
\initialscore{#4}%
  ...
  \NewDocumentCommand{\idxprima}{mmmm}{
  %% #1 :  the indexed name of the piece
  %% #2 :  mode
  %% #3 : differentia
  %% #4   name of the score file
...
\grechangestyle{initial}{\fontsize{28}{28}\selectfont}%
 ...
 %%leaving as is but I need conditionals for the following 2 commands to work as intended
  \NewDocumentCommand{\anscore}{O{}mO{}m}{%    %%you can also omit the mode and it typesets correctly.
 \greannotation{#1 Ant.}%
\greannotation{#2. #3}%
\grechangestyle{initial}{\fontsize{28}{28}\selectfont}%
...
\NewDocumentCommand{\idxan}{mO{}mO{}m}{
    \index[A]{#1 #3%
    }%
\anscore[#2.]{#3}[#4]{#5}
%% #1 indexed name of piece
%% #2   is a number; only ever 1 through 5 in,[] periods taken care of with previous command (for now: there has got to be a way to fix this so that we can omit anscore #1/idxan #2and  period
%% #3   mode without differentia, no punctuation except if it's ``per.'' 
%% #4 differentia []
%% #5  gabc file name
}

\NewDocumentCommand{\idxscore}{mmmO{}m}{
  %% #1 :  office part letter A, H, V
  %% #2 :    the indexed name of the piece; name of the score file
     %% #3 : mode numeral
  %% #4 : differentia if there is one in []
  %% #5 :  gabc file
     {\index[#1]{#2 #3}%
     }
\gscore[]{#3. #4}{#5}%
...
%%most scores are antiphons, probably a perpetually impeded Mag antiphon for a commemoration or suffrage (Veni Sancte is still its own thing: this reduces ad hoc indexing calls
\NewDocumentCommand{\idxant}{mmm}{
  %% #1 :  the indexed name of the piece; name of the score file
  %% #2 :    mode numeral
     %% #3 :gabc file
     {\index[A]{#1 #2}%
     }
...
%%series of unusual antiphons
\NewDocumentCommand{\idxo}{mmm}{
  %% #1 :  number to order from day 17 to 23 (1 through 7
  %% #2 :    name (Sapientia etc)
     %% #3 :gabc file
     {\index[A]{O 0#1@O #2 2}%
     }
\gscore[Ant.]{2. D}{#3}%
 ...
 \NewDocumentCommand{\admagnifscore}{mmm}{%
  \greannotation{Ad Magnif.}%
\greannotation{Ant. #1. #2}%
\grechangestyle{initial}{\fontsize{28}{28}\selectfont}%
 \gregorioscore{partitions/#3}%
 }

\NewDocumentCommand{\idxadmagnif}{mmmm}{
    \index[A]{#1 #2}%
\admagnifscore{#2}{#3}{#4}
%% #1 indexed name of piece
%% #2   mode without differentia, no punctuation: period is inserted with \admagnifscore: this prevents punctuation from being added to index
%% #3 differentia
%% #4 gabc file name
}


\NewDocumentCommand{\idxmag}{mmmm}{
\admagnificat{}
  %% #1 :    the indexed name of the piece; name of the score file
     %% #2 : mode numeral 1-8
  %% #3 : differentia 
...
\NewDocumentCommand{\idxsmall}{mmm}{
    \index[A]{#1 #2}%
\smallscore{#3}%
%% #1 indexed name of piece
%% #2  mode without differentia, no punctuation except if it's ``per.''
%% #3  gabc file name
}
...
\setlength{\headheight}{14pt} % prevent "headheight too small" warning

% ============ Header State ============
\newcommand{\lecustomheader}{} % Last header shown on even pages
\newcommand{\rocustomheader}{} % Last header shown on odd pages

% ============ Define Header Style ============
\NewDocumentCommand{\headerstyle}{m}{%
  \small\scshape\addfontfeature{LetterSpace=5.0}#1%
}

% ============ Headers ============
\fancyhead[LE]{%
  \thepage\hspace{0.5cm}\headerstyle{\lecustomheader}%
}
\fancyhead[RO]{%
  \headerstyle{\rocustomheader}\hspace{0.5cm}\thepage%
}

% ============ Set Header Command ============
\NewDocumentCommand{\cheader}{m}{%
  \ifodd\value{page}
    \gdef\lecustomheader{\rocustomheader}% keep old for LE
    \gdef\rocustomheader{#1}% update RO
  \else
    \gdef\rocustomheader{\lecustomheader}% keep old for RO
    \gdef\lecustomheader{#1}% update LE
  \fi
  \markboth{}{} % clear chapter/section auto headers
}

%Delay header change to the next page when it gets it wrong
\NewDocumentCommand{\ncheader}{m}{%
  \afterpage{%
    \cheader{#1}%
  }%
...
%this command still needs work and is basically deprecated
...
\NewDocumentCommand{\idxlitdate}{mm}{%
\index[F]{#1%
}%
\smalltitle{Die #2.}%
}
...
\NewDocumentCommand{\idxdominicadate}{mm}{%
\index[F]{#1%
}%
\smalltitle{Dominica #2.}%
}
...
\NewDocumentCommand{\idxferiadate}{mm}{%
\index[F]{#1%
}
\smalltitle{Feria #2.%
}%
...
\NewDocumentCommand{\idxdatefeast}{mmm}{%
\index[F]{#1%
}
  \par
  \noindent
  ...
  \makebox[\textwidth]{\Large #3.%
 }%
 }
  
  \NewDocumentCommand{\idxspecialdatefeast}{mm}{%
  \index[F]{#1%
}%
  \par
  \noindent
  ...
  %%break line as needed and the remaining text is the 3rd argument
   \NewDocumentCommand{\idxlongdatefeast}{mmmm}{%
   \index[F]{#1%
}%
  \par
  \noindent
  \makebox[0pt][l]{#2.}%
  \makebox[\textwidth]{\Large #3}%
\hfill\makebox[\textwidth]{\large #4.}%
}
...
\NewDocumentCommand{\hymnusspecial}{O{}O{}}{%
...
%%HA rubrics
\NewDocumentCommand{\harubrique}{}{%
\rubrique{In fine Hymnorum ad Horas dicitur:}%
}

\NewDocumentCommand{\infinediciitur}{m}{%
\rubrique{Hymnus \normaltext{#1,} ut in Antiphonario, sed in fine dicitur doxologia:}%
}
