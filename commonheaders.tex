...
\NewDocumentCommand{\idxinscore}{mmmm}{
  ...
  %% #2 :  name of the score file
  %% #3 : :office part letter H, A, V
  %% #4   mode
...
\grechangestyle{initial}{\fontsize{28}{28}\selectfont}%
 \gregorioscore{partitions/#4}%
     ...
     \makeatletter
\NewDocumentCommand{\idxan}{mmmO{}O{}}{
  %% this prevents page breaks between the phantom section and its label, and the actual score.
  \needspace{4\baselineskip} 
  \protected@edef\@currentlabelname{#1}
  \phantomsection
...
%% #4 is an number; only ever 1 through 5 (unfortunately it follows mode
%% #5 differentia; the last 2 arguments are in odd places but they are the ones in []
}
\makeatother
...
\NewDocumentCommand{\idxadmagnif}{mmmm}{
  ...
  \label{#2}{%
    ...
    \index[A]{#1 #3}%
\admagnifscoretest{#3}{#4}{#2}
%% #1 indexed name of piece
%% #2  gabc file name
%% #3 mode without differentia, no punctuation: period is inserted with \admagnifscore: this prevents punctuation from being added to index
%% #4 differentia
}
\makeatother
 
  \makeatletter
\NewDocumentCommand{\idxscore}{mmmmO{}}{
  %% #1 : the indexed name of the piece;
  %% #2 :  name of the score file
     %% #3 : office part letter A, H, V
  %% #4 :mode numeral
  %% #5 : differentia if there is one in []
